...
if [[ -f "$FRAGMENT_SRC" ]]; then
  ...
  echo "ERROR: Missing fragment at $FRAGMENT_SRC"
  exit 100
fi
...
if [[ -d "$OVERLAY_SRC" ]]; then
  ...
  echo "ERROR: Missing overlay directory at $OVERLAY_SRC"
  exit 100
fi

chmod +x "$BR_DIR/seedsigner_overlay/etc/init.d/S10udev" 2>/dev/null || true
chmod +x "$BR_DIR/seedsigner_overlay/etc/init.d/S50usbdevice" 2>/dev/null || true
chmod +x "$BR_DIR/seedsigner_overlay/etc/init.d/S99_auto_reboot" 2>/dev/null || true
