...
export APPLY_FRAGMENT_SCRIPT="${WORKSPACE_ROOT}/.github/scripts/apply_buildroot_fragment_and_overlay.sh"
...
apply_required_buildroot_config() {
    ...
    if [[ ! -x "$APPLY_FRAGMENT_SCRIPT" ]]; then
        print_error "Required apply script not found/executable: $APPLY_FRAGMENT_SCRIPT"
        exit 1
    fi

    "$APPLY_FRAGMENT_SCRIPT" "$WORKSPACE_ROOT" "$LUCKFOX_SDK_DIR"
