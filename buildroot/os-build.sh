    ...
    if [[ -f "$toolchain_dir/env_install_toolchain.sh" ]]; then
        ...
        if [[ "$expected_ok" -ne 1 ]]; then
            ...
            if ! command -v "$actual_cc" >/dev/null 2>&1 || ! "$actual_cc" -mabi=aapcs-linux -x c -E /dev/null >/dev/null 2>&1; then
                actual_cc=$(find "$toolchain_dir/bin" -maxdepth 1 -type f -name '*gcc' | while read -r cc; do
            ...
            local tool
            for tool in gcc g++ ld ld.bfd ar as nm objcopy objdump ranlib strip readelf; do
