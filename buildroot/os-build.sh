...
create_sd_board_artifacts() {
    local base_image="$1"
    local image_dir="$LUCKFOX_SDK_DIR/output/image"
    local board_targets="${SD_BOARD_TARGETS:-mini,max}"

    cd "$image_dir"

    IFS=',' read -ra boards <<< "$board_targets"
    for board in "${boards[@]}"; do
        board="$(echo "$board" | xargs)"
        [[ -n "$board" ]] || continue

        local board_image="${base_image%.img}-${board}.img"
        cp -v "$base_image" "$board_image"
        cp -v "$board_image" "$OUTPUT_DIR/"
        print_success "SD image created for board '${board}': $OUTPUT_DIR/$board_image"
    done
}

create_nand_image_artifacts() {
    print_step "Creating NAND-Flashable Image Artifacts"
    ...
    # Preferred NAND/eMMC flashing artifact: SDK-generated update.img
    # This is board-aware (partition map + metadata generated by the SDK firmware packaging).
    if [[ -f "update.img" ]]; then
        local nand_update_image="seedsigner-luckfox-pico-nand-update-${ts}.img"
    ...
    # Optional secondary raw-combined image for tooling that expects a monolithic dd-style image.
    if [[ -f "$nand_builder" ]]; then
        ...
        if [[ ${#missing_parts[@]} -eq 0 ]]; then
            local nand_raw_image="seedsigner-luckfox-pico-nand-raw-${ts}.img"
...
CONFIGMENU
    ...
    create_sd_board_artifacts "$IMAGE"
